...
const CreateCustomerModal = ({ isVisible, onClose, onCustomerCreated }) => {
    ...
    const createCustomerMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            // Update React Query cache for customers
...
const InstallmentConfirmationModal = ({ isVisible, onClose, onOptionSelected }) => {
    const [selection, setSelection] = useState('yes'); // 'no' or 'yes'
...
const fetchData = async (endpoint, API_TOKEN) => {
    try {
        ...
        if (!response.ok) {
            ...
            let errorMessage = `Failed to fetch ${endpoint} (Status: ${response.status})`;

            // 💡 SOLUTION: Check if the response is JSON before parsing
            if (contentType && contentType.includes("application/json")) {
                try {
                    const errorData = await response.json();
                    errorMessage = errorData.message || errorMessage;
                } catch (e) {
                    // This handles cases where the server claims JSON but sends garbage
                    console.error("Failed to parse JSON error response:", e);
                }
            } else {
                // Server returned non-JSON content (e.g., HTML like "<!DOCTYPE...")
                const errorText = await response.text();
                // Use the first 100 characters of the unexpected content in the error
                errorMessage += `. Unexpected content received. Snippet: ${errorText.substring(0, 100)}`;
            }

            // Throw a single, structured error
            throw new Error(errorMessage);
        }

        // Successfully parse the expected JSON data
        const json = await response.json();
        return json.data || [];
    } catch (error) {
        // The catch block now handles all errors, including the one thrown above
        console.error(`Error fetching ${endpoint}:`, error);
        toast.error(`Could not load ${endpoint}. Check console for details.`);
        return [];
    }
};
    ...
    const { data: customers = [], isLoading: loadingCustomers } = useQuery({
        queryKey: ['customers'],
        queryFn: () => fetchData('customers', API_TOKEN),
        enabled: !!API_TOKEN,
    });

    const { data: branches = [], isLoading: loadingBranches } = useQuery({
        queryKey: ['branches'],
        queryFn: () => fetchData('branches', API_TOKEN),
        enabled: !!API_TOKEN,
    });

    const { data: employees = [], isLoading: loadingEmployees } = useQuery({
        queryKey: ['employees'],
        queryFn: () => fetchData('employees', API_TOKEN),
        enabled: !!API_TOKEN,
    });

    const { data: products = [], isLoading: loadingProducts } = useQuery({
        queryKey: ['products'],
        queryFn: () => fetchData('products', API_TOKEN),
        enabled: !!API_TOKEN,
    });

    const { data: services = [], isLoading: loadingServices } = useQuery({
        queryKey: ['services'],
        queryFn: () => fetchData('services', API_TOKEN),
        enabled: !!API_TOKEN,
    });

    const isLoading = loadingCustomers || loadingBranches || loadingEmployees || loadingProducts || loadingServices;

    const lookupData = useMemo(() => ({
        customers: customers || [],
        branches: branches || [],
        employees: (employees || []).filter(e => e.branch_id),
        products: products || [],
        services: services || [],
    }), [customers, branches, employees, products, services]);

    const [formData, setFormData] = useState({
        customer_id: "",
        sales_type: "order", // This will be 'order' for standard sales, or used to build payload for 'installment'
        // branch_id: "",
        // source_me_id: "", 
        down_payment: 0,
        total: 0,
    ...
    // ... (Customer Update, Data Fetching, Filtering, Calculations - Unchanged) ...
    ...
    const filteredEmployees = lookupData.employees.filter(
        (emp) => Number(emp.branch_id) === Number(formData.branch_id)
    );
    ...
    useEffect(() => {
        ...
        // Auto-calculate total down payment based on selected items
        const totalDownPayment = formData.items.reduce((sum, item) => {
            const selectedItem = selectableItems.find(
                si => si.id.toString() === item.item_id.toString() && si.type === item.item_type
            );
            const itemDownPayment = selectedItem ? Number(selectedItem.down_payment || 0) : 0;
            return sum + (Number(item.qty) * itemDownPayment);
        }, 0);
    ...
    }, [formData.items, calculateGrandTotal, selectableItems]);
    ...
    // ----------------------------------------------------------------------
    const handleConfirmSaleClick = (e) => {
        e.preventDefault(); 
        ...
        setShowInstallmentConfirmation(true);
    };
...
const handleInstallmentOption = async (selection) => {
   ...
   console.log(salesOrderId)
    if (selection === 'yes') {
        ...
        navigate('/agent-create-installment', { state: { salesOrderId: salesOrderId } });
    ...
    // ----------------------------------------------------------------------
    // MODIFIED handleSubmit function (Only runs for standard sales order)
    // ----------------------------------------------------------------------
    ...
    const handleSubmit = async (e) => {
        ...
        const isItemsValid = !formData.items.some(item => !item.item_id || item.qty <= 0 || item.unit_price < 0);
        if (!formData.customer_id ||  !isItemsValid) {
            toast.error("Validation error during submission. Check customer, branch, and item details."); 
    ...
    return (
        <div className="flex h-screen overflow-hidden">
                        ...
                        <div className="bg-white rounded-2xl shadow-xl w-full p-0">
                            ...
                            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-6 rounded-t-2xl flex justify-between items-center">
                                <h2 className="text-2xl font-extrabold">
                                    Sales Order Details 
                                    {isLoading && <span className="text-sm font-normal ml-3 p-1 px-2 rounded-full bg-white bg-opacity-30">Loading Data...</span>}
                                </h2>
                            </div>
                                ...
                                <form onSubmit={handleConfirmSaleClick} className="space-y-8"> 
                                    ...
                                    >
                                        ...
                                        <div>
                                            ...
                                            <select name="customer_id" value={formData.customer_id} onChange={handleChange} className="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-200" required>
                                            ...
                                            <div key={index} className="grid grid-cols-12 gap-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm items-end">
                                                <div className="col-span-12 sm:col-span-5">
                                                    ...
                                                    <select
                                                        ...
                                                        className="w-full text-sm border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-200"
                                                        required
                                                    >
                                    ...
                                    >
                                        <div>
                                            ...
                                            <input 
                                                ...
                                                readOnly 
                                                className="w-full text-lg font-bold border-gray-300 rounded-lg shadow-sm bg-gray-100 cursor-not-allowed focus:outline-none focus:ring-0 focus:border-gray-300" 
                                                min="0" 
                                                max={formData.total} 
                                            />
                                            <p className="text-xs text-indigo-600 font-medium mt-1">
                                                * This value is automatically calculated based on the selected items.
                                            </p>
                                        </div>
                                        <div className="text-right">
                                            ...
                                            <p className="text-sm font-medium text-gray-700 mt-2">Remaining Balance (BDT):</p>
                                            <p className="text-2xl font-bold text-red-600">
                                                {(formData.total - Number(formData.down_payment)).toLocaleString()}
                                            </p>
                                        </div>
                                    </fieldset>
            ...
            <InstallmentConfirmationModal
                ...
                onOptionSelected={handleInstallmentOption}
            />
        </div>
    );
};

export default AgentCreateSale;