          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea 
                ...
                className="w-full mt-1 p-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none min-h-[80px]" 
            ...
            <div className="flex gap-3 pt-4">
              {status === 'pending' && (
                <>
                  <button 
                    onClick={() => reviewMutation.mutate({ action: 'approve', body: { notes } })}
                    disabled={reviewMutation.isPending}
                    className="flex-1 bg-green-600 text-white py-2.5 rounded-lg text-xs font-bold hover:bg-green-700 transition disabled:opacity-50"
                  >
                    {reviewMutation.isPending ? "Processing..." : "Approve"}
                  </button>
                  <button 
                    onClick={() => reviewMutation.mutate({ action: 'reject', body: { notes } })}
                    disabled={reviewMutation.isPending}
                    className="flex-1 bg-red-600 text-white py-2.5 rounded-lg text-xs font-bold hover:bg-red-700 transition disabled:opacity-50"
                  >
                    Reject
                  </button>
                </>
              ...
              {status === 'approved' && (
                <>
                  <button 
                    onClick={() => {
                      if(!confirmNumber) return toast.warning("Please provide a transaction ID");
                      reviewMutation.mutate({ action: 'paid', body: { confirm_number: confirmNumber, notes } });
                    }}
                    disabled={reviewMutation.isPending}
                    className="flex-1 bg-indigo-600 text-white py-2.5 rounded-lg text-xs font-bold hover:bg-indigo-700 transition disabled:opacity-50"
                  >
                    {reviewMutation.isPending ? "Updating..." : "Mark as Paid"}
                  </button>
                  <button 
                    onClick={() => reviewMutation.mutate({ action: 'reject', body: { notes } })}
                    disabled={reviewMutation.isPending}
              ...
              {(status === 'paid' || status === 'rejected') && (
                <div className="w-full py-2 bg-gray-100 text-gray-500 text-center rounded text-xs font-bold italic uppercase">
                  Log is {status} (Finalized)
                </div>
              )}
            </div>
          </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-[#1976D2]">
                      <tr>
                        {["ID", "User", "Amount", "Method", "Status", "Date", "Actions"].map(head => (
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-[#4338CA]">
                      <tr>
                        {["Date", "User", "Source", "Type", "Amount", "Balance"].map(head => (
                          <th key={head} className="px-6 py-4 text-left text-[11px] font-black uppercase text-white tracking-widest">{head}</th>
                        ))}
                      </tr>
                    </thead>
