...
const EditSalesOrderForm = ({ closeModal, onOrderUpdated, lookupData, BASE_URL, token, orderId }) => {
    ...
    const updateOrderMutation = useMutation({
        ...
        onError: (err) => toast.error(`Error updating order: ${err.message}`)
    });
    ...
    if (isLoadingData || !formData) {
        return (
            <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
                ...
                <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl z-10 p-8 text-center">
                    <h2 className="text-xl font-bold text-blue-600">Loading Order #{orderId} Details...</h2>
                ...
                <div className="p-8">
                    <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        <div className="flex justify-end pt-4 border-t">
                            <button 
                                type="submit" 
                                disabled={updateOrderMutation.isLoading || isLoadingData}
                            ...
                            >
                                {updateOrderMutation.isLoading ? "Updating..." : "Update Sales Order"}
                            </button>
                        </div>
                    </form>
                </div>
...
const handlePaymentClick = (row) => {
    ...
    const { data: salesOrdersData, isLoading: loading, refetch: refetchOrders } = useQuery({
        queryKey: ["serviceSales", currentPage, perPage, token],
        queryFn: async () => {
    ...
    const { data: lookupDataResult } = useQuery({
        queryKey: ["serviceSaleLookup", token],
        queryFn: async () => {
            if (!token) return {};
            const endpoints = [
            ...
            <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden bg-gray-100"> 
                ...
                <main className="grow p-4 sm:p-6 md:p-8">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 p-6 
                        ...
                        <h2 className="text-3xl font-extrabold mb-4 sm:mb-0 drop-shadow-md">SALES SERVICE ORDER MANAGEMENT</h2>
                        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                            ...
                            <Link 
                            to="/create-service-sale" // <-- Use 'to' attribute for the destination path
                            className="flex items-center justify-center bg-gradient-to-r from-green-400 to-blue-400 text-white 
                                px-5 py-2.5 rounded-xl font-bold hover:from-green-500 hover:to-blue-500 transition-all duration-300 shadow-lg"
                        >
                            <AiOutlinePlus size={20} className="mr-2" />
