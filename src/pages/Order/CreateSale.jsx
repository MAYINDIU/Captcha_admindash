...
const CreateCustomerModal = ({ isVisible, onClose, onCustomerCreated, BASE_URL, API_TOKEN }) => {
    ...
    const createCustomerMutation = useMutation({
        mutationFn: async (newCustomer) => {
            const res = await fetch(`${BASE_URL}customers?per_page=10000`, {
                method: "POST",
                headers: {
...
const CreateSale = () => {
    ...
    const { data: lookupData = { customers: [], branches: [], employees: [], products: [], services: [] }, isLoading } = useQuery({
        ...
        queryKey: ["createSaleLookup", API_TOKEN],
        queryFn: async () => {
            ...
            const [customers, branches, employees, products, services] = await Promise.all([
                fetchData('customers?per_page=10000'),
                fetchData('branches'),
                fetchData('employees'),
                fetchData('products'),
                fetchData('services'),
    ...
    const handleCustomerDataUpdate = useCallback((newCustomer) => {
        if (newCustomer) {
            queryClient.setQueryData(["createSaleLookup", API_TOKEN], (oldData) => {
                                ...
                                <form onSubmit={handleConfirmSaleClick} className="space-y-8"> 
                                        ...
                                        {formData.items.map((item, index) => (
                                            <div key={index} className="grid grid-cols-12 gap-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm items-end">
                                                <div className="col-span-12 sm:col-span-5">
                                                    <label className="block text-xs font-medium text-gray-500 mb-1">Select Product <span className="text-red-500">*</span></label>
                                                    <Select
                                                        className="basic-single"
                                                        classNamePrefix="select"
                                                        isLoading={isLoading}
                                    ...
                                    >
                                        {/* <div>
                                            ...
                                            <input type="number" name="down_payment" value={formData?.down_payment} onChange={handleChange} className="w-full text-lg font-bold border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-200" min="0" max={formData.total} />
                                            <p className="text-xs text-gray-500 mt-1">Must be less than or equal to the Grand Total.</p>
                                        </div> */}
                                        <div>
                                        ...
                                        <input 
                                            ...
                                            // onChange is no longer needed if it's not editable, 
                                            // but keeping it won't hurt if your logic requires the handler
                                            readOnly 
